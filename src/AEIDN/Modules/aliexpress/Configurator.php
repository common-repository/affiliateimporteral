  ...
  class AEIDN_AliexpressConfigurator extends AEIDN_AbstractConfigurator
  {
    ...
    protected function configureFilters()
    {
      ...
      $this->addFilter('volume', ['volume_from', 'volume_to'], 32, [
        ...
        'description' => 'from 1 to 100 <div class="sliderHolder"><div id="slider-volume"></div></div>',
    ...
    /**
     * Sync categories with AliExpress
     */
    public function syncCategories()
    {
      $html = file_get_contents('https://www.aliexpress.com/all-wholesale-products.html');
      $dom = new DOMDocument();
      libxml_use_internal_errors(true);
      $dom->preserveWhiteSpace = false;
      @$dom->loadHTML($html);
      libxml_use_internal_errors(false);
      $root = $dom->getElementById('anchor1')->parentNode->parentNode->parentNode;
      $categories = array(
      );
      // for each one we extract ID, level and name
      foreach ($root->getElementsByTagName('a') as $link) {
        $level = null;
        if ($link->parentNode->tagName === 'li') {
          $level = 2;
        } elseif ($link->parentNode->tagName === 'h3') {
          $level = 1;
        }
        $name = $link->nodeValue;
      ...
      file_put_contents(AEIDN_ROOT_PATH . '/data/aliexpress_categories.json', json_encode($categories));
    }
