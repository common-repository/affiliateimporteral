    ...
    class AEIDN_AliexpressLoader extends AEIDN_AbstractLoader
    {
        ...
        public function loadList($filter, $page = 1)
        {
            ...
            if ($link_category_id && ((isset($filter['aeidn_productId']) && !empty($filter['aeidn_productId'])) || (isset($filter['aeidn_query']) && !empty($filter['aeidn_query'])) || (isset($filter['category_id']) && (int)$filter['category_id'] !== 0))) {
                ...
                $full_request_url = apply_filters(
                    'aeidn_get_localized_url',
                    $request_url . $request_param . $request_sort,
                    array(
                        'type' => 'aliexpress_request'
                    )
                );
              ...
              WHERE external_id='" . $item['productId'] . "'");
                ...
                } elseif ((int)$error_code === 20030120) {
                    $result['error'] = 'Error! Discount input parameter error';
                } elseif ((int)$error_code === 20030130) {
                    $result['error'] = 'Error! Volume input parameter error';
                } elseif ((int)$error_code === 20030140) {
                    $result['error'] = 'Error! Page number input parameter error';
                } elseif ((int)$error_code === 20030150) {
                    $result['error'] = 'Error! Page size input parameter error';
                }
            } else {
                if ((isset($filter['aeidn_productId']) && !empty($filter['aeidn_productId'])) || (isset($filter['aeidn_query']) && !empty($filter['aeidn_query'])) || (isset($filter['category_id']) && (int)$filter['category_id'] !== 0)) {
                    ...
                    $result["error"] = 'Please enter keywords.';
                }
            }
