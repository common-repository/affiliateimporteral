...
if (!class_exists('AEIDN_SettingsPage')) {
  ...
  class AEIDN_SettingsPage
  {
    public function render()
    {
      ...
      if (
        isset($_GET['action']) &&
        $_GET['action'] === 'aliexpress_sync_categories'
      ) {
        $api = aeidn_get_default_api();
        $api->syncCategories();
      ...
      if (isset($_POST['setting_form'])) {
        ...
        if ($current_api_module === 'common') {
          ...
          update_option('aeidn_default_type', isset($_POST['aeidn_default_type']) ? sanitize_text_field($_POST['aeidn_default_type']) : 'simple');
          ...
          if (isset($_POST['aeidn_default_status'])) {
            update_option('aeidn_default_status', $_POST['aeidn_default_status']);
          }
