...
if (!class_exists('AEIDN_Ajax')) {
  ...
  class AEIDN_Ajax
  {
    public function __construct()
    {
      ...
      add_action('wp_ajax_aeidn_export_products', [$this, 'exportProducts']);
    ...
    public function exportProducts()
    {
      ...
      $outputBuffer = fopen("php://output", 'w');
      $db_res = $db->get_results('SELECT * FROM ' . $db->prefix . 'aeidn_goods');
      ...
      $db_res = $db->get_results('SELECT * FROM ' . $db->prefix . 'aeidn_goods_archive');
      if ($db_res) {
        foreach ($db_res as $row) {
          $out = (array) $row;
          $out[] = 'archive';
    ...
    public function descriptionEditor()
    {
      ...
      if ($goods->photos === '#needload#') {
        ...
        echo '<input type="button" class="save_description button button-primary" value="Save description"/>';
    ...
    /**
     * Good import.
     * Calls scheduleImportGoods after the import if it's a scheduled job.
     */
    public function importGoods()
    {
      ...
      $edit_fields = '';
      if (isset($_POST['edit_fields'])) {
        $edit_fields = sanitize_text_field($_POST['edit_fields']);
      }
      $status = 'publish';
    ...
    public function scheduleImportGoods()
    {
      ...
      try {
        ...
        if ($goods->load() && $time) {
          ...
          AEIDN_WooCommerce::addPost($goods, array(
            'import_status' => $status,
            'date' => $time,
          ));
        } else {
