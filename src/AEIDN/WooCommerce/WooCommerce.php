...
if (!class_exists('AEIDN_WooCommerce')) {
  ...
  class AEIDN_WooCommerce
  {
    ...
    public static function addPost($goods, $params = array())
    {
      ...
      foreach ($additional_meta as $meta => $value) {
        update_post_meta($post_id, $meta, $value);
      }

	  if (array_key_exists('variations', $goods)) {
        $product = wc_get_product($post_id);
        wp_set_object_terms ($post_id,'variable','product_type');
        foreach ($goods['variations'] as $option => $variants) {
          $product->set_attributes(array(
            'id' => 0, // product-level attributes
            'name' => $option,
            'options' => $variants,
            'variation' => true
          ));
	  ...
	  if (array_key_exists('quantity', $additional_meta)) {
	  ...
	  if (array_key_exists('filters', $additional_meta)) {
        update_post_meta($filters, '_aeidn_filters', $additional_meta['filters']);
      }

	  if (array_key_exists('detail_url', $additional_meta)) {
        update_post_meta($post_id, 'original_product_url', $additional_meta['detail_url']);
      }

      if (array_key_exists('ship', $additional_meta) && AEIDN_Goods::normalized($additional_meta['ship'])) {
        update_post_meta($post_id, 'ship_price', $additional_meta['ship']);
      }

      if ($additional_meta && is_array($additional_meta)) {
		if (array_key_exists('attribute', $additional_meta)) {
          AEIDN_WooCommerce::setAttributes($post_id, $additional_meta['attribute']);
        }
        if (array_key_exists('discount_perc', $additional_meta) && strlen(trim((string)$additional_meta['discount_perc'])) > 0) {
