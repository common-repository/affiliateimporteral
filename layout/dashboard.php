...
>
    <h2 class="nav-tab-wrapper">
        ...
        <?php //<a href="#" class="nav-tab nav-tab-active" rel="blacklist">Blacklist</a> ?>
    </h2>
                ...
                <div class="text_content">
                    ...
                    <table class="settings_table">
                        ...
                        foreach ($filters as $filter_id => $filter) :
                            ...
                            if($filterName=='category_id'){ continue; }
                            ?>
                            <tr>
                                <td>
                                ...
                                <td>
                                    <?php if (isset($filter['config']['type']) && $filter['config']['type'] === 'select'): ?>
                                            ...
                                            <?php if (is_array($filter['config']['data_source'])): ?>
                                                <?php foreach ($filter['config']['data_source'] as $c): ?>
                                                    ...
                                                    <?php else: ?>
                                                        <option
                                                            <?php if (isset($c['level'])): ?>class="level_<?php echo $c['level']; ?>"<?php endif; ?>
                                                            value="<?php echo $c['id']; ?>"<?php if (isset($dashboard->filter[$filterName]) && $dashboard->filter[$filterName] === $c['id']): ?> selected<?php endif; ?>>
                                                            <?php if($c['level']=='2'){ echo '- '; }
                                                            if($c['id']==''){ echo '---Select from the list---'; }else{ echo $c['name']; } ?>
                                                        </option>
                                                    <?php endif; ?>
                                        ...
                                        <button type="button" onclick="aliexpress_sync_categories()" class="button button-primary">Sync categories from AliExpress</button> - <em>will not rewrite your product categories</em>
                                    <?php else: ?>
    ...
    <div class="tab_content" rel="results">
        <?php if ($dashboard->loader->hasAccount()) : ?>
            ...
            <form method="get" id="aeidn-product-form">
    ...
    <?php /* Hide blacklist: will be added in feature release
