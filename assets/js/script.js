jQuery(document).ready(function () {
	...
	if(jQuery( "#slider-volume" ).length>0){
		jQuery( "#slider-volume" ).slider({
				...
				slide: function( event, ui ) {
					jQuery('#volume_from').val(ui.values[0]);
					jQuery('#volume_to').val(ui.values[1]);
				}
			});
		jQuery('#volume_from').val('1');
		jQuery('#volume_to').val('100');
	}
    ...
    jQuery("#aeidn-product-form .current-page").keypress(function (e) {
        if (e.which == 13) {
            ...
            queryParameters['paged'] = jQuery(this).val();
                    ...
                    if (SYNCHRONIZE_IMPORT) {
                        ...
                        jQuery.post(ajaxurl, data, function (response) {
                            ...
                            if (import_cnt_total == num_to_import) {
                                jQuery("#aeidn-goods-table .import_process_loader.completed").html("Completed! Result imported: " + import_cnt + "; errors: " + import_error_cnt + ".");
                            }
                        });
                    }
                });
    ...
    function aeidn_js_post_to_woocomerce(products_to_import, cur_index, import_cnt, import_error_cnt, import_cnt_total) {
        if (products_to_import.length > 0 && products_to_import.length > cur_index) {
            ...
            jQuery.post(ajaxurl, products_to_import[cur_index], function (response) {
                ...
                if (import_cnt_total == num_to_import) {
                    jQuery("#aeidn-goods-table .import_process_loader").addClass('completed').html("Complete! Result imported: " + import_cnt + "; errors: " + import_error_cnt + ".");
                }
    ...
    jQuery("#aeidn_price_formula").on("click", ".aeidn_edit_formula", function () {
        ...
        jQuery.post(ajaxurl, data, function (response) {
            ...
            html += '<td>';
            html += '<div class="discountJS" style="margin-bottom:5px;"><select name="discount1">';
            ...
            html += '</select></div>';
            html += '<div class="discountJS" style="margin-bottom:5px;"><select name="discount2">';
            jQuery.each(json.discount_list, function (key, value) {
